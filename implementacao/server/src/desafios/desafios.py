...
class Desafios(object):
    ...
    def gera_desafios_diario(self):
        ...
        with con:
            ...
            desafios_iniciaram_ontem = cur.execute(
                ...
                SELECT idDesafio
                 ...
                 WHERE iniciaEm = datetime(date('now', '-1 DAY'), time('10:00:00'))
                   AND apenasDoador = 1;
                """).fetchall()
            if len(desafios_iniciaram_ontem) < 2:
                cur.execute(
                    "DELETE FROM DesafiosEmAndamento WHERE iniciaEm = datetime(date('now', '-1 DAY'), time('10:00:00')) AND apenasDoador = 1;")
            ...
            if len(desafios_vao_iniciar) < 2:
                cur.execute(
                    "DELETE FROM DesafiosEmAndamento WHERE iniciaEm = datetime(date('now'), time('10:00:00')) AND apenasDoador = 1;")
                orientadores = self.shuffle_orientadores()
                desafios_todos = self.shuffle_desafios(0)
                desafios_apenas_doador = self.shuffle_desafios(1)
                i = random.randint(0, len(orientadores) - 3)
    ...
    def gera_desafios_central(self):
        ...
        with con:
            ...
            dia_da_semana = datetime.datetime.utcnow().weekday()
    ...
    def insere_desafio_central_diario(self, cursor, select, insert):
        ...
        if len(desafios_vao_iniciar) == 0:
            ...
            nome_orientador = orientadores[i]['name']
            apenas_doador = 0
            cursor.execute(insert, [id_desafio, nome_orientador, apenas_doador])
